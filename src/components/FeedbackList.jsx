        ...
        <article>
            <div className="dark:bg-slate-700 dark:text-slate-100">
                ...
                <div ref={formRef} className="pt-24">
                    <FeedbackForm editingFeedback={editingFeedback} setEditingFeedback={setEditingFeedback} />
                </div>
                {feedbacks.length === 0 ? (
                    <p className="font-semibold">Nenhum feedback disponível.</p>
                ) : (
                    feedbacks.map((feedback) => (
                        <div key={feedback.id} className="flex flex-col gap-2 border-l-2 border-orange-600 rounded-lg p-2 my-6">
                            ...
                            <p className="text-start text-sm italic mt-6">
                                <span className="font-semibold">Criado em:</span> {feedback.createdAt}
                            </p>
                            <p className="text-start text-sm italic">
                                <span className="font-semibold">Atualizado em:</span> {feedback.updatedAt}
                            </p>
                            <div className="flex gap-3">
                                <button onClick={() => handleEdit(feedback)} className="flex gap-1 items-center p-2 rounded-lg bg-orange-600 text-white">
                                    <i className='bx bxs-edit-alt'></i>
                                    <span>Editar</span>
                                </button>
                                <button onClick={() => handleDelete(feedback.id)} className="flex gap-1 items-center p-2 rounded-lg bg-red-500  text-white">
